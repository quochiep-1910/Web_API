...
namespace WebAPI.Web.API
{
    ...
    public class ApplicationGroupController : ApiControllerBase
    {
        ...
        public HttpResponseMessage Details(HttpRequestMessage request, int id)
        {
            if (id == 0)
            {
                return request.CreateErrorResponse(HttpStatusCode.BadRequest, nameof(id) + " bắt buộc.");
            }
            ...
            if (appGroup == null)
            {
                return request.CreateErrorResponse(HttpStatusCode.NoContent, "không có nhóm");
            }
