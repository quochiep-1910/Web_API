(function (app) {
    ...
    productImportController.$inject = ['$scope', '$http', 'apiService', 'authenticationService', 'notificationService', '$state', 'commonService'];
    //$state dùng để điều hướng
    function productImportController($scope, $http, apiService, authenticationService, notificationService, $state, commonService) {
        $scope.files = [];
        $scope.categoryId = 0;
        $scope.ImportProduct = ImportProduct;
        $scope.flatFolders = [];

        //bắt sự kiện trong fileUploadDiretive truyền ra
        $scope.$on("fileSelected", function (event, args) {
            $scope.$apply(function () {
                //thêm đối tượng tệp vào bộ sưu tập tệp của #scope
                $scope.files.push(args.file);
            });
        });

        function ImportProduct() {
            authenticationService.setHeader();//set token
            $http({
                method: 'POST',
